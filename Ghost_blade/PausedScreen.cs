...
namespace Ghost_blade
{
    internal class PausedScreen
    {
        ...
        public PausedScreen(GraphicsDevice graphicsDevice, Microsoft.Xna.Framework.Content.ContentManager content)
        {

            background = new AnimatedTexture(Vector2.Zero, 0f, 1f, 0f);
            background.Load(content, "Spotgame-Sheet", 6, 1, 8);

            buttonTexture = new Texture2D(graphicsDevice, 1, 1);
            buttonTexture.SetData(new[] { Color.White });
            playButton = new Rectangle(785, 800, 350, 140);
            exitButton = new Rectangle(785, 600, 350, 140);
        }
        ...
        {
            background.DrawFrame(spriteBatch, new Vector2(600, 60));
            //spriteBatch.Draw(buttonTexture, playButton, Color.Green);
            //spriteBatch.Draw(buttonTexture, exitButton, Color.Green);
        }
